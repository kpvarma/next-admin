...
import { AreaChart, Area, CartesianGrid, Legend, XAxis, YAxis, Tooltip, ResponsiveContainer } from "recharts";
import { ChartTooltipContent } from "@/components/ui/chart";
import { formatXVal, formatYVal } from '@/utils/chartUtils';

export function TimeSeriesAreaChart({ chartData, metadata }: { chartData: any[]; metadata: any }) {
  ...
  return (
    <ResponsiveContainer width="100%" height={(config.height || 200)}>
      <AreaChart data={chartData} margin={{ left: 12, right: 12 }}>
        ...
        {/* Draw Area Chart with Colors & Smooth Curves */}
        {metadata.measures.map((item, index) => (
          <Area
            ...
            fill={colors[index % colors.length]} // Fill area with corresponding color
            strokeWidth={2}
            fillOpacity={0.2} // Ensures transparency for better readability
