...
export function TimeSeriesStackedBarChart({ chartData, metadata }: { chartData: any[]; metadata: any }) {
  ...
  return (
    <ResponsiveContainer width="100%" height={(config.height || 200)}>
      <BarChart data={chartData} margin={{ left: 12, right: 12 }}>
        ...
        {/* Stack Bars Properly with Colors */}
        {metadata.measures.map((item, index) => (
          <Bar
            key={item.value}
            dataKey={item.value}
            stackId="a" // Stacks all bars together
            fill={colors[index % colors.length]} // Uses colors from chartConfig
            radius={[4, 4, 0, 0]}
